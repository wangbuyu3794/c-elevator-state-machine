...
| V5.1 | 进行中 | 新增终端可视化电梯井面板 |
| V5.2 | 进行中 | 新增紧凑终端可视化面板 |
...
| V6.1 | 进行中 | 将 Python Web 图形界面改为中文显示 |
...
当前版本先实现一个终端可视化面板，作为正式 GUI 前的界面原型。

已支持：

- 新增 `Elevator_PrintVisualPanel`；
- 菜单新增 `26`，用于打印终端可视化面板；
- 从 `34` 楼到 `-1` 楼显示电梯井；
- 使用 `[E]` 标记当前电梯所在楼层；
- 使用 `U` 显示外部上行请求；
...
- 使用 `C` 显示电梯内部楼层请求；
- 显示当前层门打开/关闭和锁定/解锁状态；
- 显示状态、方向、总时间、电源、紧急呼叫、是否可移动和是否有请求。
...
- 新增 `Elevator_PrintCompactVisualPanel`；
- 菜单新增 `27`，用于打印紧凑终端面板；
- 紧凑面板只显示当前楼层附近、目标楼层、安全楼层、救援楼层和存在请求的楼层；
- 使用省略行表示中间被隐藏的楼层；
- 顶部显示当前楼层、目标楼层、状态、方向、时间和安全摘要；
- 更适合后续观察单步运行过程。
...
- 菜单新增 `28`，用于运行一步后立刻打印紧凑终端面板；
- 保留原有 `2` 的纯运行一步行为；
- 保留原有 `27` 的只查看紧凑面板行为；
- 方便观察状态机每一步的楼层、方向、目标、门状态和请求变化。
...
- 菜单新增 `29`，用于连续运行到空闲，并在每一步后打印紧凑终端面板；
- 如果运行中遇到安全限制，会停止连续运行并打印当前紧凑面板；
- 保留原有 `3` 的纯连续运行行为；
- 适合观察多个楼层请求、方向选择、停靠和开关门流程。
...
- 菜单新增 `30`，用于连续运行到空闲，并在每一步清屏后打印紧凑终端面板；
- Windows 下使用 `cls` 清屏；
- 非 Windows 环境下使用 ANSI 清屏序列；
- 如果运行中遇到安全限制，会停止刷新式运行并打印当前紧凑面板；
- 比 `29` 更接近简单动画效果。
...
- 当电梯当前没有方向时，优先选择最近的内部楼层请求；
- 如果没有内部楼层请求，再优先选择与前往方向兼容的外部请求；
- 例如从 `1` 楼出发，同时存在 `3` 楼下行、`6` 楼上行和 `8` 楼内部请求时，电梯会先确定上行任务；
- 上行途中仍可顺路服务 `6` 楼上行请求；
- 不再因为 `3` 楼下行请求距离更近，就先向上去接一个想下行的外部乘客。
...
当前版本将 Python Web 图形界面改为中文显示。

已支持：

- 页面标题、区域标题和按钮文字改为中文；
- 状态字段名称改为中文；
- 电梯状态、方向、门状态、故障类型和事件结果显示为中文；
- 电梯井请求标记从 `U / D / C` 改为 `上 / 下 / 内`；
- 保持 C 核心逻辑不变，只在 Python 显示层做翻译。
...
- `26` 查看终端可视化面板；
- `27` 查看紧凑终端可视化面板；
- `28` 运行一步并自动查看紧凑终端可视化面板；
- `29` 一直运行到空闲并每一步自动查看紧凑终端可视化面板；
- `30` 一直运行到空闲并刷新式查看紧凑终端可视化面板；
- `0` 退出程序。
